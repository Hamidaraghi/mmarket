...
promises.push( this.storage.get("userLoginInfo").then((userLoginInfo) => {
    ...
    this.WooCommerce.getAsync("products?search=" + this.searchQuery+"&status=publish").then((searchdata => {
      this.ngZone.run(() => {
  ...
  loadMoreProducts(event) {
    ...
    this.WooCommerce.getAsync("products?search=" + this.searchQuery + "&status=publish&page=" + this.page).then((searchData) => {
      this.ngZone.run(() => {
