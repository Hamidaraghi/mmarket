...
var HomePage = /** @class */ (function () {
    ...
    HomePage.prototype.getFeaturedProducts = function (event) {
        ...
        //this.showSpinner();
        this.WooCommerce.getAsync("products?featured=true&status=publish&page=" + this.featuredProductsPage).then(function (data) {
            var acquiredProducts = JSON.parse(data.body);
            console.log(acquiredProducts);
            _this.ngZone.run(function () {
                _this.featuredProducts = _this.featuredProducts.concat(acquiredProducts);
                console.log("Featured Products are: ", _this.featuredProducts);
            });
            if (event != null && acquiredProducts.length < 10) {
                //getting last sale products again
                _this.featuredProductsPage--;
            }
        }, function (err) {
            console.log(err);
            //this.hideSpinner();
    ...
    HomePage.prototype.getSaleProducts = function (event) {
        ...
        this.WooCommerce.getAsync("products?on_sale=true&in_stock=true&status=publish&page=" + this.saleProductPage).then(function (data) {
            var acquiredProducts = JSON.parse(data.body);
            console.log(acquiredProducts);
            _this.ngZone.run(function () {
                _this.saleProducts = (_this.saleProducts).concat(acquiredProducts);
                console.log("SaleProducts are: ", _this.saleProducts);
            });
            // if (event != null) {
            //   event.complete();
            // }
    ...
    HomePage.prototype.loadMoreProducts = function (event) {
        ...
        this.WooCommerce.getAsync("products?status=publish&page=" + this.page).then(function (data) {
            var acquiredProducts = JSON.parse(data.body);
            console.log(acquiredProducts);
            _this.ngZone.run(function () {
                _this.moreProducts = (_this.moreProducts).concat(acquiredProducts);
                console.log("More Products are: ", _this.moreProducts);
            });
            if (event != null) {
                event.complete();
            }
